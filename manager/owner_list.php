...
$sql_total = "SELECT COUNT(DISTINCT users.id) AS total FROM users WHERE users.user_type = 'owner'";
$stmt_total = $pdo->prepare($sql_total);
$stmt_total->execute();
$total_records = $stmt_total->fetchColumn();
$total_pages = ceil($total_records / $records_per_page);

// Query to get list of owners and count of taxis they own with pagination
$sql = "
    SELECT users.id, users.name, COUNT(taxis.id) AS taxi_count
    FROM users
    LEFT JOIN taxis ON users.id = taxis.owner_id
...
<section class="about section-padding prelative" id="taxis" data-scroll-index='1'>
  ...
  <div class="row">
      ...
      <div class="sectioner-header text-center">
          <h3>Taxi Owners and Their Taxis</h3>
        ...
        <tbody>
            <?php foreach ($owners as $owner): ?>
                <tr>
                    ...
                    <td><?php echo htmlspecialchars($owner['taxi_count']); ?></td>
                </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
